...
{{ Form::open(array('url' => "admin/tax/save", 'id' => 'taxForm', 'name' => 'taxForm', 'class' => '','files'=>'true')) }}
												...
												<input type="hidden" name="taxval_id" value="{{ $tax['taxval_id'] }}">
												<input type="hidden" name="tax_name_cid" value="{{ $tax['tax_name_cid'] }}">
												<input type="hidden" name="tax_desc_cid" value="{{ $tax['tax_desc_cid'] }}">
        ...
        $lang_id = $content_table->lang_id;
         ...
         @endphp
																...
																<div class="row">
																	<div class="col mb-3">
																		<div class="form-group">
																			<label>Tax Description <span class="text-red">*</span></label>
																			<textarea name="tax_desc" class="form-control" >{{ $tax['tax_desc'] }}</textarea>
															...
															<option value="{{ $country->id }}" @if(($country->id == $tax['country_id'])) {{ "selected" }} @endif>{{ $country->country_name }}</option>
															@endforeach
															</select>
															</div>
															@error('country')
															...
															<option value="{{ $state->id }}" @if(isset($tax['state_id'])) @if(($state->id == $tax['state_id'])) {{ "selected" }} @endif  @endif  >{{ $state->state_name }}</option>
															@endforeach
															</select>
															</div>
															@error('state')
															...
															<input class="form-control" name="valid_from" value="{{ $tax['valid_from'] }}" type="text" readonly />
															...
															data-date-format="yyyy-mm-dd">
															<input class="form-control"  name="valid_to" value="{{ $tax['valid_to'] }}" type="text" readonly />
															<span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
															</div>
															@error('valid_to')
															<p style="color: red">{{ $message }}</p>
															...
															<input type="number" step="1.0" min="0" max="100" value="{{ $tax['percentage'] }}" name="percentage" class="form-control">
															@error('percentage')
															<p style="color: red">{{ $message }}</p>
															@enderror
															</div>
																...
																<div class="col">
																		<div class="form-group">
																			...
																			{!! Form::select('is_active', array('1' => 'Active', '0' => 'Inactive'), $tax['is_active'],['class' => 'form-control','required','id'=>'module_status']); !!}
																		</div>
																	</div>
																</div>
															</div>
