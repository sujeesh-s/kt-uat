...
@if($attrs && count($attrs) > 0) @foreach($attrs as $atr)
    ...
    @endphp
@endforeach @endif
<input type="checkbox" name="assosi[{{$row->id}}]" id="assosi_{{$row->id}}" data-val="{{$cbVal}}" value="1" class="cb {{$cbVal}} d-none" />
@endforeach @endif
    <div class="card-body">
                        ...
                        @if($assPrds && count($assPrds) > 0) @php $n = 0; @endphp
                            @foreach($assPrds as $row) @php $n++; $dtVal = ''; @endphp <?php // echo 'ssdss<pre>'; print_r($row->prdType); echo '</pre>'; die; ?>
                                @if($attrs && count($attrs) > 0) @foreach($attrs as $atr)
                                    ...
                                    @endphp
                                @endforeach @endif
                                <tr class="dtrow {{$dtVal}}" id="dtrow-{{$row->id}}" data-val="{{$dtVal}}">
