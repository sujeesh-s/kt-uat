...
<div class="row mt-4">
	...
	@if($rewards && count($rewards['all_types']) > 0)
				...
				<input min="1" step="1" type="number" name="reward_points[{{ $row['id'] }}]" id="points_{{ $row['id'] }}" class="form-control"  @if($rewards['rwd_type'] == $row['id'] || $rewards['rwd_type'] == 3)  value="{{ $row['points'] }}" @endif />
				</div>
				@endif
...
"reward_points[1]": {
required: "When Register Points is required."
},
point_val: {
