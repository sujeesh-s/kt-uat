...
<div class="page-header">
    <div class="page-leftheader">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active list-li" aria-current="page"><a href="#">Order List</a></li>
...
<div id="content_list">@include('sales.order_request.list.content')</div>
...
<script>
    ...
    $(document).ready(function(){ 
        ...
        $('body').on('click','.viewDtl',function(){ 
            ...
            $.ajax({
                type: "GET",
               url: '{{url("sales/order")}}/'+id+"/request",
        ...
        $('body').on('click','.acceptBtn',function(){
            ...
            if(status       ==  'accepted'){ var desc   =   'Order accepted by Seller'; }else{ var desc = 'Order cancellation initiated by Seller'; }
            swal({
                title: "Order status change Confirmation",
                text: "Are you sure want to "+$(this).text()+" this order?",
                // type: "input",
                ...
                if (inputValue == true) { 
                    updateStatus(id,'',status,url,'sales.order_request','order_status',smsg,desc);
                }
            });
        });
        ...
        $.ajax({
            ...
            data: { 
                    "_token": "{{csrf_token()}}", id: id, value: status,field: field, field, page: row, reply: $('.modal-content #reply').val(), model: rowId,
                     title: $('.modal-content #title').val(), start_date: $('#start_date').val(), end_date: $('#date_date').val(),type: 'request', desc: desc
                },
            success: function (data) {
                if(field == 'is_deleted'){ 
    ...
    function statusEmail(id){
        $.ajax({
            type: "POST",
            url: "{{url('send/order/status/email')}}",
