...
@php $row = 0; @endphp
<div id="list_form"> @include('earning.list.content')</div>
<div id="dtl_form">  </div>
@section('js') 
...
<script src="{{URL::asset('admin/assets/js/datatable/tables/settlement-datatable.js')}}"></script>
    ...
    $(document).ready(function(){ 
        ...
        $('body').on('click','#settleData',function(){ 
    //        $('#exampleModal .modal-content').html('<div class="modal-body">'+$('#loader').html()+'<div class="clr"></div></div>');
            var id      =   $(this).data('seller'); 
            $.ajax({
                type: "GET",
                url: '{{url("/settlements")}}/'+id,
                success: function (data){ $('#exampleModal .modal-content').html(data); } 
            }); 
        });
        
        $('body').on('click','.payBtn',function(){ 
            var id          =   $(this).data('seller'); 
            var back_page   =   $(this).data('page');
          //  $('#large-modal').modal('hide');
            $.ajax({
                type: "GET",
                url: '{{url("/payment")}}/'+id,
                data: {page: 'earning',back_page: back_page},
                success: function (data) { 
                    $('#exampleModal').modal('hide'); $('#small-modal .modal-content').html(data); 
                //    setTimeout(function(){ $('#small-modal').modal('show'); $('#small-modal .modal-content').html(data); }, 300);
                } 
            });
        });
        ...
        $('body').on('submit','#adminForm',function(e){  
            ...
            var bPage   =   $('#adminForm #back_page').val()
             ...
             //   $('#adminForm #tab-1').trigger('click');
                $.ajax({
                    ...
                    success: function (data) { 
                        ...
                        $('#small-modal').modal('hide'); toastr.success("Payment Added Successfully!");
                       if(bPage == 'settlements'){  $('body #settleData').trigger('click'); $('#exampleModal').modal('show'); }
                    }
                });
            } return false;
        });
