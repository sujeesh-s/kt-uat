...
<div class="page-header">
    ...
    <div class="page-rightheader">
        <div class="btn btn-list">
            <button style="" data-backdrop="static" class="btn btn-primary" data-toggle="modal"  data-target="#getsupport" data-container=""><i class="fa fa-plus mr-1"></i>Get support</button>
        </div>
    </div>
...
<div id="content_list">@include('seller.support_seller.list')</div>
...
//Create
        $('body').on('click','.createbtn',function(){
            ...
            $.ajax({
                type: "GET",
               url: '{{url("support/create")}}/'+id+"/view",
                success: function (data) {
     ...
     $('body').on('click','.getsupport',function(){
       // e.preventDefault(); 
        ...
        if(s_subject=='')
        {
            $('.error_subject').text('Required field');
        }

        if(s_msg=='')
        {
            $('.error_msg').text('Required field');
        }

       if(s_subject!='' && s_msg!='')
       {
        $('#table_body').append($('#loader').html()); $('#attribute').addClass('blur');
        $('#getsupport').modal('hide'); 
       //alert(msg);
       $.ajax({
           type: "POST",
           url: '{{url("support/")}}',
           data: {s_subject:s_subject, s_msg:s_msg,viewType:'ajax'},
           success: function(data) {
               //alert( msg );
