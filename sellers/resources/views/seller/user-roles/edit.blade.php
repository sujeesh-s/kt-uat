...
{{ Form::open(array('url' => "/user-roles/save", 'id' => 'userForm', 'name' => 'userForm', 'class' => '','files'=>'true')) }}
 ...
 @if($modules && count($modules) > 0)
                    @foreach($modules as $row)
                        ...
                        @php $trows =  DB::table('usr_seller_role_action')->where('usr_role_id',$userrole->id)->where('module_id',$pt['id'])->where('is_deleted',0)->where('is_active',1)->first();  @endphp
    ...
    <span class="switch-inactive" aria-hidden="true">Inactive</span>
		...
		<span class="switch-inactive" aria-hidden="true">Inactive</span>
                            ...
                            @if($child && count($child) > 0) 
                                ...
                                @php $ctrows =  DB::table('usr_seller_role_action')->where('usr_role_id',$userrole->id)->where('module_id',$ch['id'])->where('is_deleted',0)->where('is_active',1)->first();  @endphp
                                    <div class="row">
