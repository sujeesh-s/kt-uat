...
{{ Form::open(array('url' => "/auctions/save", 'id' => 'auctionForm', 'name' => 'userForm', 'class' => '','files'=>'true')) }}
												...
												<input type="hidden" name="auction_desc_cid" value="{{ $auction['auction_desc_cid'] }}">
  ...
  $def_lang =DB::table('glo_lang_lk')->where('is_active', 1)->first();
        $content_table=DB::table('cms_content')->where('cnt_id', $auction['auction_desc_cid'])->where('lang_id', $def_lang->id)->first();
        if($content_table){ 
         ...
         @endphp
																	...
																	<div class="col">
																		<div class="form-group">
																			...
																			<option value="{{ $row['category_id'] }}" <?php if($auction['cat_id']==$row['category_id']){ echo "selected";}?>>{{ $row['cat_name'] }}</option>
																			@endforeach
																			@endif
																			</select>
																		</div>
																...
																<div class="row">
																	<?php if(isset($auction['subcat_id'])){ $subcatval = $auction['subcat_id']; }else { $subcatval =''; }  ?>
																	<input type="hidden" id="prodsel" value="{{ $subcatval }}">
																	
																	<div class="col">
																		<div class="form-group">
																	
																			<label class="form-label">Subcategory <span class="text-red">*</span></label>
																			<input type="text" id="sub-category-id" placeholder="Type to filter" name="subcat_id" value="{{ $subcatval }}" autocomplete="off" hidden />
																			 <input type="text" id="sub-category-drop" class="form-control " value="{{ $auction['subcat_name'] }}" placeholder="Select Subcategory" readonly style="background-color: #fff !important;">
																		</div>
																	...
																	<div class="col">
																		<div class="form-group">
																	<?php if(isset($auction['product_id'])){ $prod_val = $auction['product_id']; }else { $prod_val =''; }  ?>
																			<label class="form-label">Product <span class="text-red">*</span></label>
																			<input type="text" id="product_id" placeholder="Type to filter" name="product_id" 
																			value="{{ $prod_val }}" autocomplete="off" hidden />
																			 <input type="text" id="product-drop" class="form-control " value="{{ $auction['product_name'] }}" placeholder="Select Product" readonly style="background-color: #fff !important;">
																		</div>
																...
																<div class="row">
																	<div class="col">
																		<div class="form-group">
																			...
																			{!! Form::textarea('auction_desc',  $auction['auct_desc'], ['class' => 'form-control','rows' => 3,'id'=>'auction_desc']) !!}
																		</div>
																		@error('auction_desc')
																	<p style="color: red" class="error">{{ $message }}</p>
																	@enderror
																...
																<div class="row">
																	...
																	<div class="col">
																		<div class="form-group">
																			...
																			<input class="form-control" name="auct_start" type="text" readonly  onchange="date_check()" value="{{ date('Y-m-d',strtotime($auction['auct_start'])) }}" />
																			<span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
																			</div> -->

<div class="input-group date" id="auct_start" data-target-input="nearest">
<input type="text" class="form-control datetimepicker-input" name="auct_start" data-target="#auct_start"  value="{{ ($auction['auct_start']) }}" />
...
<div class="input-group-text"><i class="fa fa-calendar"></i></div>
																	...
																	<div class="col">
																		<div class="form-group">
																			...
																			data-date-format="yyyy-mm-dd">
																			<input class="form-control"  name="auct_end" type="text" readonly  value="{{ date('Y-m-d',strtotime($auction['auct_end'])) }}" onchange="date_check()" />
																			<span class="input-group-addon"><i class="glyphicon glyphicon-calendar"></i></span>
																			</div> -->
<div class="input-group date" id="auct_end" data-target-input="nearest">
<input type="text" class="form-control datetimepicker-input" name="auct_end" data-target="#auct_end" value="{{ ($auction['auct_end']) }}"  />
																...
																<div class="row">
																	...
																	<div class="col">
																		<div class="form-group">
																			...
																			{!! Form::number('min_bid_price', $auction['min_bid_price'], ['class' => 'form-control','required','id'=>'min_bid_price']) !!}
																		</div>
																		@error('min_bid_price')
																	<p style="color: red" class="error">{{ $message }}</p>
																	@enderror
																	...
																	<div class="col">
																		<div class="form-group">
																			...
																			{!! Form::select('is_active', array('1' => 'Active', '0' => 'Inactive'), $auction['is_active'],['class' => 'form-control','required','id'=>'coupon_status']); !!}
																		</div>
																		@error('is_active')
																	<p style="color: red" class="error">{{ $message }}</p>
																	@enderror
...
$(function () {     
    var sminDate = $('[name="auct_start"]').val();
...
var eminDate = $('[name="auct_end"]').val();
     if(moment(sminDate) > moment()) {
          eminDate = moment();
      }
