...
class SellerEarningController extends Controller{
    ...
    public function settlements(Request $request,$sellerId=0){ 
        ...
        if($sellerId > 0){
            $data['settlements']    =   Settlement::where('seller_id',auth()->user()->id)->where('is_deleted',0)->orderBy('id','desc')->get();
                                        return view('earning.settlement',$data);
        }else{ 
            $data['settlements']    =   SalesOrder::where('payment_status','success')->groupBy('seller_id')->orderBy('id','desc')->get();
            if(isset($post->type)   ==  'ajax'){ return view('settlement.list',$data); }else{ return view('settlement.page',$data); }
        }
    ...
    function payment(Request $request, $sellerId){
        ...
        return view('earning.payment',$data);
    }
    ...
    function saveSettltment(Request $request){
        ...
        return view($post->page.'.list',$data);
    }
}
