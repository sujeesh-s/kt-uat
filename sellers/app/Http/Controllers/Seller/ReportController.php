...
class ReportController extends Controller
{
    ...
    public function coupon_report(Request $request)
    {
        ...
        $visitor      =  CouponHist::join('sales_orders','coupon_usage_hist.coupon_id','=','sales_orders.coupon_id')->join('coupon','coupon_usage_hist.coupon_id','=','coupon.id')->select('*',DB::raw('count(coupon_usage_hist.user_id) as users'),DB::raw('sales_orders.seller_id as sale_seller'))->selectRaw("sum(sales_orders.total) as purchase")->selectRaw("sum(sales_orders.discount) as discount")->whereNotIn('sales_orders.order_status',['refunded','cancelled'])->where('sales_orders.seller_id',auth()->user()->id);
    ...
    public function wishlist_report(Request $request)
    {
        ...
        $data['start_date']         =   ''; $data['end_date'] =   ''; $data['p_status'] =   ''; $data['o_status'] =   '';
        $data['seller']             =   '';
         $seller_products = Product::where('seller_id',auth()->user()->id)->where('is_active',1)->where('is_deleted',0)->where('is_approved',1)->pluck('id');
    ...
    public function review_report(Request $request,$id='',$type='')
    {
        ...
        $data['product']            = Product::where('is_approved',1)->where('is_deleted',0)->where('seller_id',auth()->user()->id)->get();
