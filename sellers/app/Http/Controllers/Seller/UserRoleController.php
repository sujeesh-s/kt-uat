...
class UserRoleController extends Controller
{
    ...
    // user roles and modules
       ...
       public function userRole()
        ...
        $data['userroles'] = SellerUserRole::getUserRoles();
        // dd($data);
        return view('seller.user-roles.list',$data);
        }
        ...
        // dd($data);
        return view('seller.user-roles.create',$data);
        }
        ...
        $data['userrole']              =   SellerUserRole::where('id',$role_id)->first();
        $data['actions']              =    DB::table('usr_seller_role_action')->where('usr_role_id',$role_id)->get();
        $data['modules']              =   Modules::getModules();
        
        return view('seller.user-roles.edit',$data);
        }
        ...
        $data['actions']              =    DB::table('usr_seller_role_action')->where('usr_role_id',$role_id)->get();
        $data['modules']              =   Modules::getModules();
        return view('seller.user-roles.view',$data);
        }
        ...
        'usr_role_name'   =>  [
        'required',
        Rule::unique('usr_seller_role_lk')->ignore($input['id']),
        ],
        ...
        $UserRoles = SellerUserRole::where('id',$input['id'])->update($roles_arr);

        $data['title']              =   'User Roles';
        $data['menu']               =   'user-roles';
        $data['userroles']              =   SellerUserRole::where('is_deleted',NULL)->orWhere('is_deleted',0)->get();

        if($input['id']  ) {
            if( $input['module_changed'] ==1) {
        DB::table('usr_seller_role_action')->where('usr_role_id',$input['id'])->update(array('is_deleted'=>1,'is_active'=>0));
        // dd($input);
        ...
        DB::table('usr_seller_role_action')->insert($actions);
        }
        }
        ...
        'usr_role_name'   =>  [
        'required',
        Rule::unique('usr_seller_role_lk'),
        ],
        'usr_role_desc' => ['required', 'string']
        ], [], 
        [
        ...
        $roles_arr = [];
        $roles_arr['org_id'] = 1;
        $roles_arr['seller_id'] = auth()->user()->id;
        $roles_arr['usr_role_name'] = $input['usr_role_name'];
        $roles_arr['usr_role_desc'] = $input['usr_role_desc'];
        ...
        $UserRoles = SellerUserRole::create($roles_arr);
        $lastId = $UserRoles->id;
...
if(isset($input['modules'])) {
        ...
        return redirect(route('seller.user-roles'));
        ...
        if($input['id']>0) {
            $role_id = $input['id'];
        $deleted =  SellerUserRole::where('id',$input['id'])->update(array('is_deleted'=>1,'is_active'=>0));
        DB::table('usr_seller_role_action')->where('usr_role_id',$input['id'])->update(array('is_deleted'=>1,'is_active'=>0));
        StaffInfo::where('staff_role_id',$role_id)->update(array('is_active'=>0));
        ...
        if($input['id']>0) {
        $deleted =  SellerUserRole::where('id',$input['id'])->update(array('is_active'=>$input['status']));
