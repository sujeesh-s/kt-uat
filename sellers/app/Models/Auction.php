...
'shipping_cost_id','auct_start','auct_end','bid_allocated_to','sale_id','is_active','is_deleted','created_by','updated_by','created_at','updated_at'];
        ...
        static function getAuctionData($acn_id){ 
           ...
           $acn_list = Auction::where("id",$acn_id)->orderBy('id', 'DESC')->get();     
            ...
            if(Product::where('id',$row->product_id)->first()) { $data['product_name']        =  Product::where('id',$row->product_id)->first()->name; }else { $data['product_name']        = ""; }
            if(ProductImage::where('prd_id',$row->product_id)->first()){
