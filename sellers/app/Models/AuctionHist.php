...
class AuctionHist extends Authenticatable
{
  ...
  static function getLog($acn){ 
        $log_list = AuctionHist::where('auction_id',$acn)->orderBy('id', 'DESC')->get();
           ...
           if(SalesOrder::where("id",$row->sale_id)->first()){ $data[$row->id]['sale_code']         =  SalesOrder::where("id",$row->sale_id)->first()->order_id; }else { $data[$row->id]['sale_code'] ='';}
          if(SalesOrder::where("id",$row->sale_id)->first()){  $data[$row->id]['sale_date']         =  SalesOrder::where("id",$row->sale_id)->first()->created_at;}else { $data[$row->id]['sale_date']  = ''; }
