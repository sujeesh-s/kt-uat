...
class PrdReview extends Model{
    ...
    protected $fillable = ['prd_id','user_id','rating','comment','image','is_active','is_deleted','created_by','updated_by','user_type'];
    public function product(){ return $this->belongsTo(Product ::class, 'prd_id'); }
    public function customerinfo($user_id){ return CustomerInfo::where('user_id',$user_id)->first(); }
    ...
    static function getProductReviews($prd_id){ 
        $prd = $prd_id;
            ...
            $query->select('cust_id')->from('sales_orders')->where("seller_id",auth()->user()->id)->whereIn('id',function($query) use ($prd_id) {
