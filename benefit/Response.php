<?php
	...
	$con = mysqli_connect("estrradoweb.com","estrrado_vrise","Vrise@321","estrrado_budgetsouq");
	if ($trandata != "")
	{
		...
		if ($returnValue == 1)
		{
			...
			{
				echo("REDIRECT=https://kt.estrradoweb.com/benefit/approved.php");
			}
			else if ($Pipe->getresult() == "NOT CAPTURED" || $Pipe->getresult() == "CANCELED" || $Pipe->getresult() == "DENIED BY RISK" || $Pipe->getresult() == "HOST TIMEOUT")
			{
				...
				echo "REDIRECT=https://kt.estrradoweb.com/benefit/declined.php";
			}
			else
			{
				//Unable to process transaction temporarily. Try again later or try using another card.
                echo "REDIRECT=https://kt.estrradoweb.com/benefit/err-response.php";
			}
		}
		else
		{
